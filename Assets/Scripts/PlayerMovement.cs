...
public class PlayerMovement : MonoBehaviour
{
    ...
    private void Movement()
    {
        ...
        {
            _lastDirection = movement; // ќбновл€ем последнее направление движени€
    ...
    {
        // ”станавливаем спрайт поко€ в зависимости от последнего направлени€ движени€
        if (Mathf.Abs(_lastDirection.x) > Mathf.Abs(_lastDirection.y))
        {
            _characterSprite.sprite = idleHorizontalSprite;
            _characterSprite.flipX = _lastDirection.x > 0; // ѕереворачиваем спрайт влево
        }
        else
        {
